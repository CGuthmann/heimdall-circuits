...
template PolygonPresentation(branchingFactor, depth, revocationDepth, polygonSize) {
	/*
	* Private Inputs
	*/
	// Meta
	...
	*/
	...
	signal output inbound; // 8
	signal output index_o[width]; // 9..44
	signal output vertx_o[polygonSize]; //45..94
	signal output verty_o[polygonSize]; //95..144/*
	/*
	* Meta Calculations
	*/
	...
	*/
	...
	for (var i = 0; i < width; i++) {
		polygon.index[i] <== index[i];
		polygon.attribute[i] <== values[i];
	}	
...
component main = PolygonPresentation(6, 2, 13, 50);
